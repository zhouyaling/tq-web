    ...
    <ta-border-layout  :layout="{header:'55px'}" :center-cfg="{showBar:true}" :header-cfg="{showBorder: false}" :showBorder="false" :footer-cfg="{showBorder: false}">
      ...
      <ta-modal centered
        ...
        <div slot="title" style="text-align: center">
          给人员 [{{this.$route.query.name}}] 分配审核角色
        </div>
        <div slot="footer" style="text-align: center">
          <ta-button @click="associateVisible=false"><ta-icon type="rollback"/>返回</ta-button>
...
export default {
  ...
  data () {
    return {
      adminUserRoleUrl: 'org/authority/examinerAuthorityRestService/queryRoleByUserId', // 根据用户id查询该用户拥有的角色列表url
      adminUserModalRoleUrl: 'org/authority/examinerAuthorityRestService/queryNoWrapperRoleByUserId', // 模态框加载可以分配角色的url
      userRoleColumns, // 人员角色列属性
      operateMenu: [{
        name: '移除',
        type: 'confirm',
        confirmTitle: '确认移除该审核角色吗?',
        onOk: (record) => {
