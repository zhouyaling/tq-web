...
export default {
  ...
  data () {
    return {
      orgCascaderUrl: 'org/authority/adminAuthorityManagementRestService/queryCurrentAdminRoleWrapOrgTree', // 审核权限人员维度url
      adminUserUrl: 'org/authority/examinerAuthorityRestService/queryUserByCondition', // 表格分页查询审核人员维度列表
      casValue: [], // 级联默认数据
      orgOptions: [], // 级联数据
      isSub: true, // 是否包含子组织
      searchInfo: '', // 搜索人员信息
  ...
  methods: {
    ...
    fnRouteToRoleMg (record) {
      this.$router.push({ path: 'userRoleMg', query: { name: record.name, userId: record.userId } })
    },
    // 用户参数
    userPageParams () {
      let params = {}
